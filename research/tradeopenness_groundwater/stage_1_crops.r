...
# Read in data
df <- read.csv('results/2000_2010/input_data_clean_stage1.csv')

for (f.path in list.files(glue('rawdata/gwd_csv/2010'))) {
    crop <- unlist(strsplit(f.path,'_'))[1]
    print(crop)

    df2 <- df

    names(df2)[names(df2) == glue('{crop}')] <- 'GWD'

	lenError <- tryCatch({
        df2$GWD[df2$GWD==0] <- NA
	}, error=function(e) e)
    ...
    df2 <- subset(df2, !( (is.na(GWD)) |  (is.na(Population)) | (is.na(Rainfall)) | (is.na(Temperature)) | (is.na(lperP))  | (is.na(ckperP))  |(is.na(TO)) )  )

    obs <- nrow(df2)
    print(glue('Observations: {obs}'))

    if (length(unique(df2$ISO)) == 1) {
        print('Not enough data')
        next
    }
    
    # OLS
    m <- lm(log(GWD)~TO,data=df2)
    print('OLS...')
    print(summary(m))
    print('---')
    ...
    print('IV version 1...')
    res.v1 <- iv_solve(f.1,df2)
    print(res.v1)
    print('---')

    # IV, Version 2
    print('IV version 2...')
    res.v2 <- iv_solve(f.2,df2)
    print(res.v2)
    print('---')
