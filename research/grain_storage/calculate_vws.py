...
class VWS:
    ...
    def __init__(self, year, harv_path, prod_path, stor_path, precip_path, codes_path, bu_conv_path, harv_prod_cols, stor_cols, states_ignore, geo = 'STATE', by = 'State ANSI', wd = 'final_results'):
        ...
        ### Merge datasets ###
        self.merged_usda = self.merge_data( self.stor, self.harv, self.prod , self.by )
    ...
    def merge_data(self, stor, harv, prod, by):
        df = pandas.merge(stor, harv, on = by)
        df = pandas.merge(df, prod, on = [by,'Commodity'])
