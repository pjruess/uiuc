...
plot_vws <- function(counties, vws, commodity, year, identifier, label, mmdata, ncats=8) {
	
	# Define path to save to
	path <- sprintf('county_plots/%s/%s_%s_%s.png', tolower(commodity), year, tolower(commodity), label)
	...
	if (commodity == 'total') {
		...
		#vws <- setDT(vws)[, lapply(.SD, sum, na.rm=TRUE), by=.(GEOID,Storage_Bu), .SDcols=c('Irrigated_Harvest_Acre','Rainfed_Harvest_Acre','Irrigated_Percent_Harvest','Rainfed_Percent_Harvest','Yield_Bu_per_Acre','Production_Bu','CWU_bl_m3ha','CWU_gn_ir_m3ha','CWU_gn_rf_m3ha','CWU_bl_and_gn_ir_m3ha','VWS_ir_m3','VWS_rf_m3','VWS_m3','Precipitation_km','Precipitation_Volume_km3','Capture_Efficiency')]
		#setDT(vws)
		...
		vws <- data.frame(vws)
		write.csv(vws,sprintf('county_plot_aggregates/%s_aggregate_%s_data.csv',year,label),row.names=FALSE)
	} else {
	...
	#write.csv(map.df,sprintf('county_plots/TEST_%s_aggregate_%s_data.csv',year,label),row.names=FALSE)
...
minmax.path <- 'county_plots/minmax_database.csv'
...
for (c in commodities) {
	for (i in 1:nrow(df)) {
		for (y in years){
			...
			tryCatch({
			       	minmax.data <- plot_vws(counties_data, vws_data, c, y, id, label, minmax.data, ncats=8)
