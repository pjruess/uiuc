...
path = 'results/county_flows_no_duplicates.csv'
...
if os.path.isfile(path):
    ...
    df1 = pandas.read_csv('data/cnty_faf12_domestic_1_0.0005_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df2 = pandas.read_csv('data/cnty_faf12_domestic_2_0.005_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df3 = pandas.read_csv('data/cnty_faf12_domestic_3_0.05_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df4 = pandas.read_csv('data/cnty_faf12_domestic_4_0.005_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df5 = pandas.read_csv('data/cnty_faf12_domestic_5_5e-05_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df6 = pandas.read_csv('data/cnty_faf12_domestic_6_5e-05_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    df7 = pandas.read_csv('data/cnty_faf12_domestic_7_5e-05_mincost_upload.csv',usecols=['sim_ori','sim_des','simulate'],quotechar='"')
    
    # Reformat for proper merging
    df1['simulate'] = pandas.to_numeric(df1['simulate'])
    df2['simulate'] = pandas.to_numeric(df2['simulate'])
    df3['simulate'] = pandas.to_numeric(df3['simulate'])
    df4['simulate'] = pandas.to_numeric(df4['simulate'])
    df5['simulate'] = pandas.to_numeric(df5['simulate'])
    df6['simulate'] = pandas.to_numeric(df6['simulate'])
    df7['simulate'] = pandas.to_numeric(df7['simulate'])
    ...
    #print 'SCTG 1 shape (non-zero):',df1[df1['simulate'] != 0].shape
    #print 'SCTG 2 shape (non-zero):',df2[df2['simulate'] != 0].shape
    #print 'SCTG 3 shape (non-zero):',df3[df3['simulate'] != 0].shape
    #df3[df3['simulate'] != 0].to_csv('TEST_orig.csv')
    #print 'SCTG 4 shape (non-zero):',df4[df4['simulate'] != 0].shape
    #print 'SCTG 5 shape (non-zero):',df5[df5['simulate'] != 0].shape
    #print 'SCTG 6 shape (non-zero):',df6[df6['simulate'] != 0].shape
    #print 'SCTG 7 shape (non-zero):',df7[df7['simulate'] != 0].shape
    ...
    #df3[df3['simulate'] != 0].to_csv('TEST_SPEC_orig.csv')
    ...
    df12.rename(columns={'simulate_x':'sctg_1','simulate_y':'sctg_2'},inplace=True)

    # Merge with df3
    df123 = pandas.merge(df12,df3,on=['ori_des'],how='outer')
    df123.rename(columns={'simulate':'sctg_3'},inplace=True)
    
    # Merge with df4
    df1234 = pandas.merge(df123,df4,on=['ori_des'],how='outer')
    df1234.rename(columns={'simulate':'sctg_4'},inplace=True)
    
    # Merge with df5
    df12345 = pandas.merge(df1234,df5,on=['ori_des'],how='outer')
    df12345.rename(columns={'simulate':'sctg_5'},inplace=True)
    
    # Merge with df6
    df123456 = pandas.merge(df12345,df6,on=['ori_des'],how='outer')
    df123456.rename(columns={'simulate':'sctg_6'},inplace=True)
    
    # Merge with df7
    df_all = pandas.merge(df123456,df7,on=['ori_des'],how='outer')
    df_all.rename(columns={'simulate':'sctg_7'},inplace=True)
