...
pdf(sprintf('figures/fig4_faf_%s.pdf',(1-quant)))
...
plot(fafzones,col='grey20',fill=TRUE,bg='black',border='black',lwd=0.1,xlim=c(-125,-66),ylim=c(24,50))
...
points(x=df$ori_lon,y=df$ori_lat,pch=16,cex=df$ori_sum/max_sum,col='honeydew3')

# Color and weighting for arcs
col.1 <- adjustcolor('honeydew3',alpha=0.3)
col.2 <- adjustcolor('honeydew3',alpha=0.7)
