...
nc.path <- 'waterdemand_30min_groundwaterabstraction_annual.nc'
...
# Get GWD variable
gwa <- ncvar_get(nc,'gwab')
dlname <- ncatt_get(nc,'gwab','long_name')
dunits <- ncatt_get(nc,'gwab','units')
fillvalue <- ncatt_get(nc,'gwab','_FillValue')
...
# Fill zeroes
gwa[gwa==fillvalue$value] <- NA

# Select rasters to compare
gwa.y1 <- gwa[,,41] #2000
gwa.y2 <- gwa[,,51] #2010
r.y1 <- raster(t(gwa.y1),xmn=min(lon),xmx=max(lon),ymn=min(lat),ymx=max(lat),crs=CRS(proj4string(usa)))
r.y2 <- raster(t(gwa.y2),xmn=min(lon),xmx=max(lon),ymn=min(lat),ymx=max(lat),crs=CRS(proj4string(usa)))
...
pdf('gwa_2010vs2000_BrBG.pdf')
# Function for plotting raster differences
plot(r.dif,xlim=c(-128,-66),ylim=c(22,52),col=brewer.pal(n=m,name='BrBG'),breaks=breakSeq,main='Difference in Groundwater Abstraction, 2010 minus 2000 [Mm3]')
#library(rasterVis)
