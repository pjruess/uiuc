...
map <- function(geo, df, c, y, i, mm) {
	...
	path <- sprintf('../maps/survey_conus/%s_%s_%s.png', y, c, l)
    print(path)
...
minmax.path <- '../maps/survey_conus/minmax_census.csv'
...
# Read in data
df <- read.csv('../data/clean/clean_census_survey_corn_grain.csv')
...
# Select only Survey data
df <- subset(df, Program == 'SURVEY')[,c(1:2,4:7)]
...
for (y in unique(df$Year)) {
	...
	for (i in colnames(df)[4:6]) {
        ...
        for (c in unique(as.character(df$Crop))){ # since only looking at Corn, Grain right now
