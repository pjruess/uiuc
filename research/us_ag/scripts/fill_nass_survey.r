...
fill <- function(path,cnty,type) {
    ...
    write.csv(df,sprintf('../data/compare_census_survey/output/survey/soybeans_survey_raw_clean_%s_allyears.csv',type),row.names=F)
    ...
    for (y in unique(df$Year)) {
        ...
        # If sum is equal to total, fill with NA values (because all unreported), otherwise fill w val
        ...
        write.csv(year.df,sprintf('../data/compare_census_survey/output/survey/soybeans_survey_filled_%s_%s.csv',type,y),row.names=F)
    ...
    write.csv(final,sprintf('../data/compare_census_survey/output/survey/soybeans_survey_filled_%s_allyears.csv',type),row.names=F)
...
for (i in c('area','prod')){
    path <- sprintf('../data/compare_census_survey/survey_soybeans_%s_93-19.csv',i)
