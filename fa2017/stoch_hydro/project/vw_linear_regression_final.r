...
lm_path = 'final_results/linear_model_data.csv'
...
write.csv(data, 'final_results/linear_model_data_steps.csv', row.names=FALSE)
...
vars = c('CWU_BL','CWU_GN_IR','CWU_GN_RF','CWU_BL_LOG','CWU_GN_IR_LOG','CWU_GN_RF_LOG','LANDAREA','PRECIPITATION','MEANTEMP','MINTEMP','MAXTEMP','DEWPOINT','MINVAPORPRES','MAXVAPORPRES','ELEVATION')

run_anova(obj='CWU_BL',vars=vars,path='final_results/anova_cwu_bl.doc')
run_anova(obj='CWU_GN_IR',vars=vars,path='final_results/anova_cwu_gn_ir.doc')
run_anova(obj='CWU_GN_RF',vars=vars,path='final_results/anova_cwu_gn_rf.doc')
run_anova(obj='CWU_BL_LOG',vars=vars,path='final_results/anova_cwu_bl_log.doc')
run_anova(obj='CWU_GN_IR_LOG',vars=vars,path='final_results/anova_cwu_gn_ir_log.doc')
run_anova(obj='CWU_GN_RF_LOG',vars=vars,path='final_results/anova_cwu_gn_rf_log.doc')
...
path = 'final_results/linear_model_cwu_bl.doc'
...
path = 'final_results/linear_model_cwu_gn_ir.doc'
...
path = 'final_results/linear_model_cwu_gn_rf.doc'
...
check.assumptions <- function(LM,ext){
	...
	png(paste('final_results/residuals_',ext,'.png',sep=''))
	...
	### TEST FOR NORMALITY ###
	# Visualize Normality
	png(paste('final_results/normality_',ext,'.png',sep=''))
	...
	capture.output(shapiro.test(LM.stdres), file=paste('final_results/shapiro_test_',ext,'.doc',sep='')) # p < alpha --> do not reject null (resid normally dist)
	...
	# for (var in vars) {
	# 	png(paste('final_results/manual_constvar_',ext,'_',var,'.png',sep=''))
	...
	png(paste('final_results/residual_plots_',ext,'.png',sep=''))
	capture.output(residualPlots(LM), file=paste('final_results/tukey_test_',ext,'.doc',sep='')) # this returns resid plot for all variables, & Tukey p-values
	dev.off()
	...
	library(lmtest)
	capture.output(dwtest(LM,alternative='two.sided'),file=paste('final_results/dwtest_',ext,'.doc',sep='')) # two.sided, greater, or less
	# Variance Inflation Factor
	vif(LM)
	capture.output(vif(LM),paste('final_results/vif_',ext,'.doc',sep='')) # Relationship between variables. Want low values; high means variables
