...
bptest.sarlm(m9.ser)
bptest.sarlm(m9.sal)
#bptest.sarlm(m9.sac) # doesn't work, must be sarlm object
jarque.bera.test(resid(m9.ser))
jarque.bera.test(resid(m9.sal))
jarque.bera.test(resid(m9.sac))
moran.test(resid(m9.ser),listw=usa.l,zero.policy=T)
moran.test(resid(m9.sal),listw=usa.l,zero.policy=T)
moran.test(resid(m9.sac),listw=usa.l,zero.policy=T)

# Fixed effects...
m9fe <- plm(formula=f9.s,data=df,index=c('State','Year'),model='within',effect='twoways')
summary(m9fe)
#fixef(m9fe)
jarque.bera.test(resid(m9fe))
bptest(m9fe)
