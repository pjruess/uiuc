...
df <- read.csv(sprintf('clean/prism_tmean_county/PRISM_tmean_stable_4kmM3_%s01_bil.csv',y))
colnames(df) <- c('ID',sprintf('tmean%s01',y))

for (m in 2:12){
    m <- sprintf('%02d',m)
    temp <- read.csv(sprintf('clean/prism_tmean_county/PRISM_tmean_stable_4kmM3_%s%s_bil.csv',y,m))
...
write.csv(res,sprintf('clean/annual/prism_tmean_county/tmean%s_county.csv',y),row.names=F)
