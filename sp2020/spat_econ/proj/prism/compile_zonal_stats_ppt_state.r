...
for (y in years){
    ...
    df <- read.csv(sprintf('clean/prism_ppt_state/PRISM_ppt_stable_4kmM3_%s01_bil.csv',y))
    colnames(df) <- c('ID',sprintf('ppt%s01',y))
    
    for (m in 2:12){
        m <- sprintf('%02d',m)
        temp <- read.csv(sprintf('clean/prism_ppt_state/PRISM_ppt_stable_4kmM3_%s%s_bil.csv',y,m))
    ...
    write.csv(res,sprintf('clean/annual/prism_ppt_state/ppt%s_state.csv',y),row.names=F)
