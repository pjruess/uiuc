...
# Space fixed effects
fe.s <- plm(formula=f7,data=df,index=c('State','Year'),model='within',effect='individual')
summary(fe.s)

# Time fixed effects
fe.t <- plm(formula=f7,data=df,index=c('State','Year'),model='within',effect='time')
summary(fe.t)
...
# Compare all models to OLS (all have sig p-val, so all are better than OLS)
pFtest(fe.s,m7)
pFtest(fe.t,m7)
pFtest(fe.all,m7)

# Random effects
rand <- plm(formula=f7,data=df,index=c('State','Year'),model='random')
summary(rand)
plmtest(rand) # sig p-val: random may be important
