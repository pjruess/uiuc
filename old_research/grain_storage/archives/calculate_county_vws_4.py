...
class alldata:
    ...
    def calculate_vws(self,dataset,path):
        ...
        if os.path.isfile(path):
            ...
            df['CWU_m3ha']  = pandas.to_numeric(df['CWU_m3ha'],errors='coerce')
            df['Storage_Bu'] = pandas.to_numeric(df['Storage_Bu'],errors='coerce')
            # df = pandas.to_numeric(df[['Percent_Harvest','Yield_Bu_per_Acre','CWU_m3ha','Storage_Bu']],errors='coerce')
            df['VWS_m3_yield'] = df['CWU_m3ha'] * ( 1. / df['Yield_Bu_per_Acre'] ) * df['Storage_Bu'] * df['Percent_Harvest'] * 0.405 # ha/acre
            df['VWS_m3_prod'] = df['CWU_m3ha'] * ( df['Harvest_Acre'] / df['Production_Bu'] ) * df['Storage_Bu'] * df['Percent_Harvest'] * 0.405 # ha/acre
        ...
        print '{0} CWU (m3):                     {1:,}'.format(self.year,long(df['CWU_m3ha'].sum()))
