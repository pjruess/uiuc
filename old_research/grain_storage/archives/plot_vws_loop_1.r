...
plot_vws <- function(counties, vws, commodity, year, identifier, label, ncats=8) {
	...
	path <- sprintf('county_plots/%s_%s_plot_%s.png', year, label, tolower(commodity))
	
	# If storage path already exists (since commodity is irrelevant), skip this plot
	if (identifier == 'Storage_Bu') {
		if (commodity != 'ALL'){ next }
		path <- sprintf('county_plots/%s_%s_plot.png', year, label)
	}
	...
	if (commodity == 'ALL') {
		...
		setDT(vws)
		write.csv(vws,sprintf('county_plots/%s_aggregate_%s_data.csv',year,label))
	} else {
