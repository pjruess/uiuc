...
plot_vws <- function(states, vws, commodity, year, identifier, label, ncats=8) {
	...
	path <- sprintf('state_log_plots/%s_%s_plot_%s.png', year, label, tolower(commodity))
	
	# If storage path already exists (since commodity is irrelevant), skip this plot
	if (identifier == 'Storage_Bu') {
		if (commodity != 'ALL'){ next }
		path <- sprintf('state_log_plots/%s_%s_plot.png', year, label)
	}
	...
	if (commodity == 'ALL') {
		vws <- setDT(vws)[, lapply(.SD, sum, na.rm=TRUE), by=.(State.ANSI,Storage_Bu), .SDcols=c('Harvest_Acre','Yield_Bu_per_Acre','Production_Bu','CWU_bl_m3ha','CWU_gn_ir_m3ha','CWU_gn_rf_m3ha','VWS_m3_yield','VWS_m3_prod','VWS_bl_m3_yield','VWS_bl_m3_prod','VWS_gn_ir_m3_yield','VWS_gn_ir_m3_prod','VWS_gn_rf_m3_yield','VWS_gn_rf_m3_prod')]
		setDT(vws)
		write.csv(vws,sprintf('state_log_plots/%s_aggregate_%s_data.csv',year,label))
	} else {
