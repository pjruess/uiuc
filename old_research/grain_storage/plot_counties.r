...
plot_vws <- function(counties, vws, commodity, year, identifier, label, ncats=8) {
	...
	if (commodity == 'ALL') {
		vws <- setDT(vws)[, lapply(.SD, sum, na.rm=TRUE), by=.(GEOID,Storage_Bu), .SDcols=c('Irrigated_Harvest_Acre','Rainfed_Harvest_Acre','Irrigated_Percent_Harvest','Rainfed_Percent_Harvest','Yield_Bu_per_Acre','Production_Bu','CWU_bl_m3ha','CWU_gn_ir_m3ha','CWU_gn_rf_m3ha','CWU_bl_and_gn_ir_m3ha','VWS_ir_m3','VWS_rf_m3','VWS_m3')]
		setDT(vws)
	...
	write.csv(map.df,sprintf('county_plots/TEST_%s_aggregate_%s_data.csv',year,label))
